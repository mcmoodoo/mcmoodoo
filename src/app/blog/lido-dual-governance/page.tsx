	...
	return (
		<>
			...
			<div className="max-w-3xl mx-auto p-6">
				...
				<p className="mb-4">
					DG aims to protect users from the negative effects of the DAO’s
					one-sided governance, allowing the stETH holders to impact DAO
					decisions and indirectly defend their interests.
				</p>
				...
				<p className="mb-4">
					Disincentivizing majority stake ownership in LDO will thwart cartel
					activities and bring more uniformity to the poor LDO token
					distribution. Owning LDO to rug stETH holders will no longer be
					relevant because the targeted users will most likely just flee with
					their funds intact at the first sign of danger and long before the
				...
				<p className="mb-4">
					...
					of defense for stakers after all the negotiations have failed. In an
					attempt to retain users, the DAO might simply decide to withdraw the
					proposal, prioritizing users above the desired protocol changes. This
					gives users more say in the governance.
				</p>
				<p className="mb-4">
					If, however, users fail to persuade the DAO to back off, they can
					resort to exiting the protocol. After they exit, the DAO resumes its
					governance and can execute the controversial proposal.
				</p>
				...
				<p className="mb-4">
					In any other extreme cases, including governance attacks or removing
					the majority of operators, this mechanism proves useless: a huge
					outflow of stETH refugees will cause big line-ups in Ethereum’s
					throughput-limited withdrawal queue and users won’t be able to leave
					in the time allotted. Moreover, users who have locked their stETH
					elsewhere for higher yields face prolonged unwinding times. Under a
					single veto phase, these users would have been stuck!
				</p>
				<p className="mb-4">
					All in all, Dual Governance will allow stakers to exit the protocol
					without being subject to new and pending DAO decisions, further
					improving foot-voting efficiency.
				</p>
				...
				<p className="mb-4">
					A quorum of users could sound an alarm and issue a veto which will
					trigger the dynamic time lock. The most active, yet honest minority
					will notice an attempt for a shady change in the governance and start
					increasing the time lock by joining the veto.
				</p>
				<p className="mb-4">
					This mechanism, therefore, will give users the ability to negotiate
					and induce the DAO to withdraw its proposal. Otherwise, the users will
					resort to foot voting and exit the protocol.
				</p>
				...
				<p className="mb-4">
					This approach worsens the situation by creating another instance of
					the Principal-Agent problem. The role of the agent is now taken by
					this newly-introduced committee which might not act in the best
					interest of users. Therefore, delegating the task of watching and
					filtering the protocol changes to the committee might not be a good
					idea. Additionally, this mechanism is quite slow and inefficient.
				</p>
				...
				<p className="mb-4">
					Well, either this time lock has to be extremely long to account for
					any withdrawal queue or there has to be an oracle that will keep an
					eye on the withdrawal queue length. Either way, this approach will
					reduce the efficiency of governance. Propagations of protocol changes
					will be delayed, specifically those that aim to fix a critical
					vulnerability. Lastly, a simple time lock won’t provide users with any
					negotiation power nor does it improve the principal-agent problem.
				</p>
				...
				<p className="mb-6">
					The drawback of this approach is that users will be burdened with
					governance duties. This goes against the users’ initial motivation to
					obtain stETH, which is mostly for the reason of collecting yield
					rather than the headache from the protocol governance. Additionally,
					allowing users to select node operators usually leads to centralizing
					the validator set.
				</p>
				<h3 className="text-xl font-bold mt-6 mb-3">
				...
				<p className="mb-6">
					Although this explainer aims to call on all token holders to vote in
					favor of Dual Governance, it’s critical to emphasize the importance of
					launching before EIP 7002. Currently, LDO holders can’t force node
					operators to exit. Even in the event of a captured DAO, it’s hard to
					coerce Ethereum-aligned operators to act against their will. However,
					EIP 7002 will drastically shift the balance of power here by allowing
					LDO holders to force operators out of the protocol. This is why Dual
					Governance ideally needs to go live before or shortly after the EIP
					7002. Users must be provided with exit guarantees to protect against a
					malicious withdrawal vault upgrade or undesirable changes to the node
					operator set.
				</p>
			</div>
		</>
